...
def box_extraction(img_for_box_extraction_path, cropped_dir_path):
    ...
    (thresh, img_bin) = cv2.threshold(img, 16, 255,cv2.THRESH_BINARY | cv2.THRESH_OTSU)  # Thresholding the image
    ...
    for c in contours:
        ...
        # If the box height is greater then 22, widht is >22, then only save it as a box in "cropped/" folder.
        if (w > 22 and h > 22) and w >= h:
            idx += 1
            new_img = img[y:y + h, x:x + w]
            new_img =  cv2.resize(new_img, (28, 28), interpolation = cv2.INTER_NEAREST)
			#temp=new_img
			#new_img = 255 - temp
